...
public class SearchTermService implements ISearchTermService {
    ...
    @Cacheable(value = "searchTerms", key = "#pageIndex + '-' + #pageSize")
    public ApiResponse<PagedResult<SearchTermDto>> getAllSearchTerms(int pageIndex, int pageSize) {
        ...
        int zeroBasedPage = Math.max(0, pageIndex - 1);

        Pageable pageable = PageRequest.of(zeroBasedPage, pageSize, Sort.by(Sort.Direction.DESC, "createdAt"));
